...
const navSections: NavSection[] = [
  ...
  {
    id: 'screens', label: 'Client Screens', icon: <Monitor size={18} />,
    children: [
      { id: 'screens-all', label: 'All Screens' },
      { id: 'screens-add-client', label: 'Add Client Screen' },
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'reports-screens', label: 'Screen Reports' },
      { id: 'reports-media', label: 'Media Reports' },
...
export default function Sidebar({ activeView, onNavigate, collapsed, onToggle, onLogout }: Props) {
  const [expandedSections, setExpandedSections] = useState<Set<string>>(
    new Set(['my-screens', 'screens', 'my-channel', 'client-management', 'licenses', 'reports', 'support', 'settings'])
  );
  ...
  const isActive = (id: string) => {
    ...
    if (id === 'reports') {
      return activeView.startsWith('reports-');
    }
    if (id === 'settings') {
      return activeView.startsWith('settings-');
    }
