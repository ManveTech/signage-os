...
export default function ManageScreens() {
  ...
  const handleBulkAction = () => {
    ...
    if (bulkAction === 'delete') {
      const updated = screens.filter(s => !selectedScreens.has(s.id));
      setScreens(updated);
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 text-gray-600 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors cursor-pointer">
            <Download size={15} />
            Export
          </button>
        </div>
