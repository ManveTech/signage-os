...
export default function CreatePlaylist({ userEmail = 'admin@demo.com', onNavigate, isMyChannel }: Props) {
  ...
  const [targetUserEmail, setTargetUserEmail] = useState<string>(() => {
    ...
    if (forClient) {
      if (forClient === 'new') {
        const licensesList = licensingStore.getLicenses().filter(l => l.assignedUserEmail);
        if (licensesList.length > 0 && licensesList[0].assignedUserEmail) {
          return licensesList[0].assignedUserEmail;
        }
      } else {
        return forClient;
      }
  ...
  const handleCompilePlaylistVideo = async (): Promise<string | undefined> => {
    ...
    try {
      ...
      toast.success('Playlist video successfully compiled with sound!');
      return videoDataUrl;
    } catch (err: any) {
  ...
  const loadPlaylists = () => {
    ...
    setExistingPlaylists(all.filter(p => p.createdBy === targetUserEmail));
  };
  ...
  const loadMedia = () => {
    ...
    const clientMedia = all.filter(m => m.uploadedBy === targetUserEmail);
    setMediaList(clientMedia);

    syncCollection('media_items', 'signageos_media').then(updatedMedia => {
      const updatedClientMedia = updatedMedia.filter(m => m.uploadedBy === targetUserEmail);
      setMediaList(updatedClientMedia);
    });

    // Get current storage usage
    const used = mediaStore.getClientStorageUsedBytes(targetUserEmail);
    setStorageUsedBytes(used);

    // Get current storage limit from license
    const licensesList = licensingStore.getLicenses();
    const lic = licensesList.find(l => l.assignedUserEmail === targetUserEmail);
    if (lic) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-slate-800">
                Prepare Playlist for Client Organization
              </p>
              <p className="text-[10px] text-gray-500">
                Choose which organization this playlist and upload limits apply to.
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-8 space-y-5">
                                ...
                                {secondMedia ? (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button 
                                      ...
                                      className="p-1 text-gray-400 hover:text-rose-500 hover:bg-rose-550 rounded cursor-pointer"
                                    >
                                      <X size={12} />
                                    </button>
                                  </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5 shadow-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              {playlistWidgetType && (isWidgetActive('qrcode') || isWidgetActive('weather') || isWidgetActive('clock')) && (() => {
                const activeSecondary = isWidgetActive('qrcode') ? 'qrcode' : isWidgetActive('weather') ? 'weather' : 'clock';
                return (
                  <div className="sm:col-span-2">
                    <label className="block text-[10px] text-slate-455 uppercase tracking-widest font-black mb-1.5">
                      {activeSecondary === 'qrcode' ? 'QR Code Link / URL' :
                       activeSecondary === 'weather' ? 'Weather Location / City' : 'Clock Label / Header'}
                    </label>
                    <input
                      type={activeSecondary === 'qrcode' ? 'url' : 'text'}
