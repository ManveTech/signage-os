...
export default function MediaLibrary({ userEmail }: Props) {
  ...
  const [storageLimitGb, setStorageLimitGb] = useState(100); // Admin gets 100GB limit by default
  ...
  const handleDeleteSelected = () => {
    ...
    showToast(`${count} media item(s) deleted`);
    loadData();
  };
    ...
    <div className="p-6 space-y-5 text-left relative">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-xs max-w-xl">
        ...
        <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden border border-gray-100">
          <div className="h-full rounded-full bg-blue-600" style={{ width: `${storageUsedPercent}%` }} />
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-500 mb-1">
                This will permanently delete the <strong>{selectedIds.length} selected media item{selectedIds.length !== 1 ? 's' : ''}</strong> matching the current filter from your library.
              </p>
