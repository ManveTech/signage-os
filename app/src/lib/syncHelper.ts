...
// Dynamically resolve local Express proxy address:
// - iOS Simulator / Web: localhost
// - Android Emulator: 10.0.2.2 (Android loopback alias)
const getLocalIP = () => {
...
export const API_BASE = `http://${getLocalIP()}:5000/api/v1`;
...
// Sync all database collections from the Express API proxy
export async function syncAllFromDatabase(): Promise<Record<string, any[]>> {
  const collections = [
  ...
  for (const col of collections) {
    try {
      const res = await fetch(`${API_BASE}/${col.path}`, { headers });
      if (res.ok) {
        const data = await res.json();
        await AsyncStorage.setItem(col.key, JSON.stringify(data));
        results[col.key] = data;
      } else {
        const cached = await AsyncStorage.getItem(col.key);
        results[col.key] = cached ? JSON.parse(cached) : [];
      }
    } catch (err) {
...
// Sync a single collection from server and update cache
export async function syncCollection(collectionPath: string, localStorageKey: string): Promise<any[]> {
  try {
    const headers = await getHeaders();
    const res = await fetch(`${API_BASE}/${collectionPath}`, { headers });
    if (res.ok) {
      const data = await res.json();
...
// Push mutations (POST/PUT/DELETE) to database
